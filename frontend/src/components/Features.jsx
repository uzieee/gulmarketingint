      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="flex flex-col md:flex-row justify-between items-end mb-16 gap-6">
          <div className="max-w-2xl">
            ...
            <h2 className="text-3xl md:text-5xl font-extrabold text-gray-900 tracking-tight leading-tight">
              Leading Medical <span className="text-transparent bg-clip-text bg-gradient-to-r from-primary-600 to-primary-800">Categories</span>
            </h2>
            <p className="mt-4 text-xl text-gray-500">
              Browse our exclusive, high-performance diagnostic machinery and instruments tailored for modern clinics.
            </p>
          </div>
          <button className="flex-shrink-0 flex items-center gap-2 text-primary-600 hover:text-primary-800 font-semibold group transition-all">
            View All Categories 
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mb-24">
          {categories.map((cat, idx) => (
            <a 
              key={idx} 
              href={cat.link}
              className="group relative rounded-3xl overflow-hidden cursor-pointer shadow-md hover:shadow-2xl transition-all duration-500 hover:-translate-y-2 bg-gray-100 min-h-[400px]"
            >
              {/* Image Background */}
              <div className="absolute inset-0 w-full h-full">
                ...
                />
                <div className={`absolute inset-0 bg-gradient-to-t opacity-80 group-hover:opacity-90 transition-opacity duration-300 via-gray-900/60 to-transparent ${cat.color} mix-blend-multiply`}></div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
          {popularItems.map((item, idx) => (
            <div key={idx} className="group flex flex-col items-center p-6 bg-white border border-gray-100 rounded-2xl hover:shadow-xl hover:border-primary-100 transition-all duration-300 hover:-translate-y-1">
              <div className="relative w-full aspect-square bg-gray-50 rounded-xl overflow-hidden mb-6 flex items-center justify-center p-4">
                ...
                <div className="absolute inset-0 bg-white/40 opacity-0 group-hover:opacity-100 backdrop-blur-[2px] transition-opacity duration-300 flex items-center justify-center">
                  <button className="bg-primary-600 text-white px-6 py-2 rounded-full font-semibold shadow-lg hover:bg-primary-700 hover:scale-105 transition-all">
                    Quick View
                  </button>
                </div>
              </div>
